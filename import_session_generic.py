#!/usr/bin/env python3
"""
Generic session importer for crypto indicators database.
"""
...
def import_session(db_path, json_file):
    ...
    if cursor.fetchone()[0] > 0:
        print(f"\nWARNING: Session {session_num} already exists in database!")
        print("Skipping import to avoid duplicates.")
        conn.close()
        return False

    # Determine category and subcategory based on session number
    if session_num <= 3:
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python import_session_generic.py <db_path> <json_file>")
        sys.exit(1)
    ...
    # Import
    success = import_session(db_path, json_file)
